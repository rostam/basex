...
public class QueryParser extends InputParser {
  ...
  /**
   * Sets the input file and checks the query for illegal length or characters.
   * @throws QueryException query exception
   */
  private void prepare() throws QueryException {
  ...
  /**
   * Finishes the parsing and checks all invariants.
   * @param main if a main module was parsed
   * @throws QueryException query exception
   */
  private void complete(final boolean main) throws QueryException {
    if(more()) {
      if(alter != null) error();
      final String rest = rest();
      ip++;
      throw main ? error(QUERYEND, rest) : error(MODEXPR, rest);
    }
   ...
   * @throws QueryException query exception
   */
  public final QNm parseModule(final byte[] u) throws QueryException {
    prepare();
    try {
      versionDecl();
      wsCheck(MODULE);
  ...
  public void module(final byte[] path, final byte[] uri) throws QueryException {
    ...
    new QueryParser(qu, ctx).parseModule(uri);
    ctx.sc = sc;
  }
  ...
  private void varDecl(final Ann ann) throws QueryException {
    ...
    ctx.globals.set(ctx, info(), vn, tp, ann, bind, true, external);
  }
  ...
  private void functionDecl(final Ann ann) throws QueryException {
    ...
    final UserFunc func = new UserFunc(ii, name, args, type, ann, scope, true);
    if(func.updating) ctx.updating(true);
    func.expr = body;
    ctx.funcs.add(func, ii);
  ...
  private Expr comparison() throws QueryException {
    ...
    if(e != null) {
      for(final OpV c : OpV.values()) if(wsConsumeWs(c.name))
        return new CmpV(info(), e, check(ftContains(), CMPEXPR), c);
      for(final OpN c : OpN.values()) if(wsConsumeWs(c.name))
        return new CmpN(info(), e, check(ftContains(), CMPEXPR), c);
      for(final OpG c : OpG.values()) if(wsConsumeWs(c.name))
        return new CmpG(info(), e, check(ftContains(), CMPEXPR), c);
    }
    return e;
  }
  ...
  private Expr extension() throws QueryException {
    ...
    return pr.length == 0 ? null : new Extension(info(), pr, enclosed(NOPRAGMA));
  }
    ...
    do {
      ...
      if(wsConsume(BR1)) {
        ...
        if(env != null) {
          ...
          scope = env.scope;
          e = new PartFunc(info(), fixScope(e), env);
          scope = scope.parent();
        }
      }
    } while(e != old);
  ...
  private Expr functionItem() throws QueryException {
    ...
    if(wsConsume(FUNCTION) && wsConsume(PAR1)) {
      ...
      scope = scope.parent();
      return new InlineFunc(info(), type, args, body, ann, inner);
    }
  ...
  private Expr functionCall() throws QueryException {
    ...
    if(name != null && !keyword(name)) {
      ...
      if(args != null) {
        ...
        if(f != null) {
          alter = null;
          return env != null ? new PartFunc(info(), f, env) : f.fun;
        }
      }
    }
